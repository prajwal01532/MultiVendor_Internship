...
const bannerSchema = new Schema({
    ...
    store: { type: Schema.Types.ObjectId, ref: 'PharmacyStore' },
    zone: { type: Schema.Types.ObjectId, ref: 'PharmacyZone' }
}, { timestamps: true });
...
const cashbackSchema = new Schema({
    title: { type: String, required: true },
    customerId: { type: Schema.Types.ObjectId, ref: 'PharmacyCustomer' },
...
const couponSchema = new Schema({
    ...
    userUsage: [{
        user: { type: Schema.Types.ObjectId, ref: 'PharmacyUser' },
        count: { type: Number, default: 0 }
    }],
    status: { type: String, enum: ['active', 'inactive', 'expired'], default: 'active' },
    store: { type: Schema.Types.ObjectId, ref: 'PharmacyStore' },
    products: [{ type: Schema.Types.ObjectId, ref: 'PharmacyProduct' }],
    categories: [{ type: Schema.Types.ObjectId, ref: 'PharmacyCategory' }],
}, { timestamps: true });
...
const notificationSchema = new Schema({
    ...
    sender: { type: Schema.Types.ObjectId, ref: 'PharmacyUser' },
...
const Banner = mongoose.model('PharmacyBanner', bannerSchema);
const Campaign = mongoose.model('PharmacyCampaign', campaignSchema);
const Cashback = mongoose.model('PharmacyCashback', cashbackSchema);
const Coupon = mongoose.model('PharmacyCoupon', couponSchema);
const Notification = mongoose.model('PharmacyNotification', notificationSchema);

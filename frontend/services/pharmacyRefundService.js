...
const refundService = {
    ...
    getRefunds: async (page = 1, limit = 10, filters = {}) => {
        try {
            ...
            const response = await axios.get(`${API_URL}/api/pharmacyorders`, {
                params: {
                    page,
                    limit,
    ...
    getRefundById: async (id) => {
        try {
            const response = await axios.get(`${API_URL}/api/pharmacyorders/${id}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
    ...
    updateRefundStatus: async (id, status, remarks = '') => {
        try {
            const response = await axios.put(
                `${API_URL}/api/pharmacyorders/${id}/status`,
                { 
